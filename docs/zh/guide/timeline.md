...
| ~1960 | [环形缓冲区](/zh/patterns/ring-buffer/) | 用于电信和实时 I/O 系统；无单一发明者 |
| ~1960 | [Arena 分配器](/zh/patterns/arena-allocator/) | 编译器中的区域分配；Knuth 在 TAOCP（1968）中讨论了池分配 |
| 1960 | [引用计数](/zh/patterns/reference-counting/) | George Collins 描述了用于自动存储回收的引用计数 |
| ~1960 | [驻留](/zh/patterns/interning/) | LISP 从最早期实现就驻留符号；该技术早于其名称 |
...
| ~1965 | [脏标记](/zh/patterns/dirty-flag/) | 虚拟内存系统使用"脏位"跟踪已修改的页面；该模式后来泛化为任何延迟重算场景 |
| 1966 | [LRU 缓存](/zh/patterns/lru-cache/) | Belady 的"虚拟存储计算机替换算法研究"（IBM 系统期刊） |
| ~1966 | [标签联合体](/zh/patterns/tagged-union/) | Algol 68 形式化了判别联合体；更早的汇编程序员非正式地使用类型标签 |
| 1967 | [虚函数表](/zh/patterns/vtable/) | Simula 67 通过方法表引入虚方法分派；C++ 后来使其流行，并使用"vtable"这一名称 |
| ~1967 | [事件循环](/zh/patterns/event-loop/) | 早期交互系统使用事件驱动分派；由 Smalltalk（1980）和 X Window 系统（1984）推广 |
| 1970 | [布隆过滤器](/zh/patterns/bloom-filter/) | Burton Bloom 发表"带允许误差的哈希编码中的空间/时间权衡"（CACM） |
| ~1970 | [B+ 树](/zh/patterns/b-plus-tree/) | Bayer & McCreight 发明 B 树（1970）；叶节点链接的 B+ 变体于 1972 年左右出现，用于数据库索引 |
...
| ~1975 | [墓碑](/zh/patterns/tombstone/) | 数据库系统中用于删除标记；对 B 树删除和后来的 LSM 树至关重要 |
...
| ~1993 | [中间件链](/zh/patterns/middleware-chain/) | 责任链（GoF 1994）泛化为 Web 框架的中间件管道；CORBA 中间件更早出现 |
| ~1993 | [注册表](/zh/patterns/registry/) | COM（1993）和 CORBA 使用注册表进行组件发现；GoF 的抽象工厂与之相关 |
...
| ~2003 | [合并迭代器](/zh/patterns/merge-iterator/) | 通过最小堆实现 K 路有序流合并；在 LevelDB/BigTable 时代系统中形式化 |
