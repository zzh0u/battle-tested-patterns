...
const message = ref('Add tasks to workers, then start processing to see work stealing');
...
function addTasks(workerIdx: number) {
  ...
  message.value = `Added 4 tasks to ${w.name} (queue: ${w.queue.length})`;
}
...
function startProcessing() {
  ...
  timer.value = setInterval(() => {
    ...
    for (const w of workers.value) {
      if (w.queue.length > 0) {
        ...
        if (busiest) {
          ...
          message.value = `${w.name} stole task #${stolen.id} from ${busiest.name}!`;
          anyWork = true;
    ...
    if (!anyWork) {
      stopProcessing();
      message.value = 'All tasks completed!';
    }
  }, 300);
}
...
function reset() {
  ...
  stealHighlight.value = '';
  message.value = 'Reset — add tasks and start processing';
}
...
<template>
  <div class="viz-container">
    <div class="viz-title">Interactive Work Stealing</div>
      ...
      >
        <div class="ws-worker-header">
          ...
          <span class="ws-worker-stats">
            queue: {{ w.queue.length }} | stolen: {{ w.stolen }}
          </span>
        </div>
        ...
        <div class="ws-queue">
          ...
          <div v-if="w.queue.length === 0" class="ws-empty">idle</div>
        </div>
    ...
    <div class="viz-controls">
      <button class="viz-btn viz-btn--primary" @click="startProcessing" :disabled="running">Start</button>
      <button class="viz-btn" @click="stopProcessing" :disabled="!running">Stop</button>
